...
||||||| parent of d011067892 (Refactor agent cache identity to use timing fields only)
...
export class AgentHookServer {
  ...
  dropPersistedStatusEntry(identity: AgentStatusCacheIdentity): boolean {
    ...
    // Reuse the ordinary dismissal semantics after the guard: it preserves a
    // resumable provider session and notifies all status consumers.
...
export class AgentHookServer {
  ...
  dropPersistedStatusEntry(identity: AgentStatusCacheIdentity): boolean {
    ...
    const existing = this.state.lastStatusByPaneKey.get(resolvedPaneKey) as
      | EnrichedAgentHookEventPayload
      | undefined
    // Why: receivedAt + stateStartedAt pin the exact event instance (same baseline the
    // interrupt-inference guard uses). Renderer-enriched fields (connectionId, worktreeId)
    // diverge from main's cache — comparing them made the eviction silently no-op.
    if (
      !existing ||
      existing.receivedAt !== identity.receivedAt ||
      existing.stateStartedAt !== identity.stateStartedAt
...
>>>>>>> d011067892 (Refactor agent cache identity to use timing fields only)
